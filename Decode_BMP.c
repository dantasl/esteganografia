  ...
  while(temporario_decimal != 3){ //por quê 3? 3 é o decimal para End of Text
    ...
    for(i = 0; i < 8; i++){
      ...
      if(decodifique_proximo == R){
        ...
        temporario_cores[i] = mensagem_binaria[7];
...
int Decode_BMP(char *argv_imagem,char *argv_output){
  FILE *imagem_original;
	imagem_original = fopen(argv_imagem, "rb");
  Imagem *img;
	if( imagem_original == NULL ){
		printf("Erro na abertura da imagem BMP original. Verifique o nome, se ela existe ou se tem algum conteúdo salvo. Depois disso, tente novamente.\n");
    exit(1);
	} else {
		img = ler_imagem_bmp(imagem_original, img);
    decodificar_imagem_bmp(img, argv_output);
	}
